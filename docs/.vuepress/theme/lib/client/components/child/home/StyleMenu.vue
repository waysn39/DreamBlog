...
<template>
  <div class="setColor"
       :style="$store.state.borderRadiusStyle + $store.state.fontFamilyStyle + $store.state.fontColorStyle">
    <div v-if="isShowIco"
         @click="clickSetColor"
         id="setIco"
         class="setIco bg_color">
    </div>
    ...
    <div class="welcome-parent"
      ...
      <div @click="clickSet"
           :class="customClass"
           :style="getColorStyle"
           class="m-4 p-4 f4 color-shadow-small bg-gray-800-mktg rounded-2 signup-content-container welcome"
           id="welcome">
        <span @click="clickSetColor"
              class="cancel aurora-iconfont-common aurora-style-cancel"
              id="cancel"></span>
        ...
        <div class="custom-top custom-common">
          <div class="custom-top-span custom-common-span">
            <span @click="setImg"
                  class="aurora-iconfont-common aurora-style-img home-welcome-custom-icon"></span>&nbsp;
          </div>
          <div class="custom-top-span custom-common-span">
            <span @click="setIsFitter"
                  class="aurora-iconfont-common aurora-style-blur home-welcome-custom-icon"></span>&nbsp;
          </div>
        </div>

        <div class="custom-bottom custom-common">
          <div style="flex: 1"
               class="custom-bottom-span custom-common-span">
            <li class="custom-li"
                v-for="(item) in fontColorArr"
                :key="item.index">
              <span :style="setColorBack(item)"
                    :data-color="item"
                    @click="setColor($event,item)"></span>
            </li>
          </div>
        </div>
        ...
        <div class="custom-bottom custom-common">
          <div style="flex: 1"
            ...
            <li class="custom-li"
                ...
                :key="item.index">
              <span :style="setFamilyBack(item)"
                    :data-color="item"
                    @click="setFont($event,item)">{{getShowFont}}</span>
            </li>
          </div>
        </div>
