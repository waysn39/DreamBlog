<template>
  <div class="home"
    ...
    <div v-if="randomSawRes"
         :class="{'home-random-say-custom': isHome}"
         class="home-random-say">
      <div v-if="isHome">
        <span>「 {{obj.output}}」</span>
      </div>
    </div>
    <slot name="home2"></slot>
    <div :class="{'home-social-custom': isHome}"
         class="home-social">
      <div :class="{'home-social-custom-single': isHome}">
        <HomeSidebarSocialItem v-for="item in socialsArrTemp"
                               :key="item.index"
                               :sidebar-row-pc-var="socialsIsHomeNum + 1"
    ...
    <div v-if="isHome"
         class="home-click-down">
      <span @click="clickDown"
            class="aurora-iconfont-common home-click-down-icon"></span>
    </div>
    <div v-if="showWave && isHome"
         :class="showHomeWaveStyle ? 'home-wave-show': 'home-wave-hide'"
         class="home-wave-box"
         ref="home-wave">
      <div class="home-wave">
        <canvas class="home-wave-canvas1"
                id="home-wave-canvas1"></canvas>
        <canvas class="home-wave-canvas2"
                id="home-wave-canvas2"></canvas>
      </div>
    </div>
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (this.themeProperty.customRandomSay) {
        this.randomSawRes =
          this.themeProperty.customRandomValue === undefined ||
          this.themeProperty.customRandomValue == null
            ? "Aurora theme"
            : this.themeProperty.customRandomValue;
